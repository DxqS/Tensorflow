...
class AdditiviGaussianNoiseAutoencoder(object):
    def __init__(self, n_input, n_hidden, tranfer_function=tf.nn.softplus,
                 optimizer=tf.train.AdamOptimizer, scale=0.1):
        self.n_input = n_input
        self.n_hidden = n_hidden
        self.transfer = tranfer_function
        ...
        self.hidden = self.transfer(
            tf.add(
                tf.matmul(self.x +
                          scale * tf.random_normal((n_input,)), self.weights['w1']), self.weights['b1']))
...
def standard_sacle(X_train, X_test):
...
X_train, X_test = standard_sacle(mnist.train.images, mnist.test.images)
n_samples = int(mnist.train.num_examples)
training_epochs = 20
batch_size = 128
display_step = 1
autoencoder = AdditiviGaussianNoiseAutoencoder(n_input=784, n_hidden=200, tranfer_function=tf.nn.softplus,
